...
class PluginConfig(system: ActorSystem, systemConfig: Config) {
  ...
  lazy val maxConnections: Int =
    if (dbConfig.hasPath("maxConnections")) dbConfig.getInt("maxConnections") else numThreads
  ...
  lazy val eventStore: Option[EventStore] = {
    PluginConfig.asOption(eventStoreConfig.cfg.getString("class")) map { storeName =>
      val storeClazz =
        Thread.currentThread.getContextClassLoader.loadClass(storeName).asInstanceOf[Class[_ <: EventStore]]
